...
//            });
        ...
        public static void ObjectRename(this FwConfig config, string oldName, string newName)
        {
            ...
            config.VirtualEDataObject.ForEach(w => w.ObjectRename(oldName, newName));
        }
        public static void ObjectRenameCode(this FwConfig config,string oldName,string newName)
        {
            ...
            config.VirtualEDataObject.ForEach(w => w.ObjectRenameCode(oldName,newName));
        }
        public static void ObjectRenameTableName(this FwConfig config, string oldName, string newName)
        {
            ...
            config.VirtualEDataObject.ForEach(w => w.ObjectRenameTableName(oldName, newName));
        }
        /// <summary>
        /// 修改对象属性代码
        /// </summary>
        ...
        {
            EDB edb = new EDB();
            edb.ETables = fc.DictObject
                .SelectMany(w => w.ToTable(fc))
                .Concat(fc.DataObjects.SelectMany(w => w.ToTable(fc)).ToList())
                .Concat(fc.SYSObject.SelectMany(w => w.ToTable(fc)).ToList())
                .ToList();
