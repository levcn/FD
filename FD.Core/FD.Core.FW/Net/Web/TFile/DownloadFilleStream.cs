...
namespace Fw.Net.Web.TFile
{
    public static class DownloadFilleStream
    {
        /// <summary>
        ...
        /// <param name="isMediaStream">如果是,名写入文件名</param>
        /// <param name="writeBefore">写数据之前</param>
        /// <returns>true下载成功，false下载失败</returns>
        public static bool DownloadFile(HttpContext httpContext, string filePath, long speed, bool isMediaStream = false, Func<byte[], byte[]> writeBefore = null)
        {
            bool ret = true;
            try
            {
...
//                string eTag = HttpUtility.UrlEncode(fileName, Encoding.UTF8);
                #endregion
                ...
                try
                {
                    ...
                    string str = fi.Extension.Replace(".", "");
                    if (str == "bmp" || str == "jpg" || str == "png" || str == "gif")
                    {
                        httpContext.Response.ContentType = "image/jpeg"; //MIME类型：匹配任意文件类型
                    }
                    else
                    {
//                        httpContext.Response.ContentType = "video/x-flv"; //MIME类型：匹配任意文件类型
                    ...
                    if (httpContext.Request.Headers["Range"] != null)
                    {
                        ...
                        if (range.Length >= 3 && !String.IsNullOrEmpty(range[2]))
                        {
                            
                            endPosition = Convert.ToInt64(range[2]);
                        }
                    }
                    httpContext.Response.AddHeader("Content-Length", (endPosition - startBytes).ToString());

                    httpContext.Response.AddHeader("Connection", "Keep-Alive");
                    if (startBytes > 0)
                    {
                        //------如果是续传请求，告诉客户端本次的开始字节数，总长度，以便客户端将续传数据追加到startBytes位置后----------
                        httpContext.Response.AddHeader("Content-Range", string.Format("bytes {0}-{1}/{2}", startBytes, endPosition - 1, endPosition));
                    }
                    else
                    {
                        //做为流媒体使用时,这个参数不能加,要不然会造成视频进度不能拖放
                        if (!isMediaStream)
                        {
                    ...
                    {//客户端中断连接，则暂停
                        byte[] buffer = br.ReadBytes(packSize);
                        if (writeBefore != null) buffer = writeBefore(buffer);
